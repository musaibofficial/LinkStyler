...
InlineKeyboardButton('Set Title ✍️',callback_data='title')],            [InlineKeyboardButton('Send Links 🔗', callback_data='links')],[InlineKeyboardButton ('Back',callback_data='back2')]
...
chat_id=call.from_user.id,text=setting, reply_markup=gen_markup2())
    ...
    bot.edit_message_text(message_id=v.id,chat_id=call.from_user.id,text=v.text,reply_markup=gen_markup())
...
@bot.message_handler(regexp=r"(?i)\b((?:https?://|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:'\".,<>?«»“”‘’]))")
...
def say_no(m):
 ...
 bot.reply_to(m, "No Links Detected Please Use /help Or /start To Get Started ") 
